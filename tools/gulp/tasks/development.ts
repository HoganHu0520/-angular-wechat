...
} from 'angular-wechat-build-tools';
...
/** Path to the output of the Angular Wechat package. */
const angularWechatOutPath = join(outputDir, 'packages', '@angular-wechat');
...
task(':watch:devapp', () => {
  ...
  watch(join(angularWechatOutPath, '**/*.css'), [':build:devapp:scss', triggerLivereload]);
});
...
task('serve:devapp', ['build:devapp'], sequenceTask(
  [':serve:devapp', 'angular-wechat:watch', ':watch:devapp']
));
...
task('stage-deploy:devapp', ['build:devapp'], () => {
  ...
  copyFiles(angularWechatOutPath, '**/prebuilt/*.+(css|map)', join(outDir, 'dist/packages/angular-wechat'));
});
